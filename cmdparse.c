...
cmd_free(command_t *cmd)
{
	...
	while( tmp != NULL ){
		...
		for (int i = 0; i < 3; i++){
			if (tmp->redirect_filename[i] != 0){
				free(tmp->redirect_filename[i]);
			}
		}

		if (tmp -> subshell){
			cmd_free(tmp->subshell);}
		next = tmp -> next;
		free(tmp);
		tmp = next;
	}
...
cmd_parse(parsestate_t *parsestate)
{
	...
	while (1) {
		...
		token_t token;
		parse_gettoken(parsestate, &token); //Fetches the next token from the input line.
		...
		case TOK_NORMAL:
			...
			i++;
			token_t token_test;
			parse_gettoken(parsestate, &token_test);
			if (token_test.type == TOK_OPEN_PAREN){
				goto error;
			}
			parse_ungettoken(parsestate);
			break;
		...
		case TOK_OPEN_PAREN:;
			int in_pars = 1;
			command_t *cmd_subshell_list;
			cmd_subshell_list = cmd_line_parse(parsestate, in_pars);
			cmd->subshell = cmd_subshell_list;
...
cmd_line_parse(parsestate_t *parsestate, int in_parens)
{
	...
	command_t *head = NULL;
	command_t *cmd;
	token_t token; //The token's string value is stored in 'buffer'.
	int r;
	...
	while (1) {
		...
		cmd = cmd_parse(parsestate); //Parses a single command_t structure from the input string.
